...
export function blockedTags(state) {
  return [...new Set(activeInjuries(state).flatMap(x=>Array.isArray(x.blockedTags)?x.blockedTags:[]))];
}
...
export function sessionLoadSeries(state) {
  return (state.sessions||[]).filter(s=>s.completedAt&&Number.isFinite(Number(s.trainingLoad))&&Number(s.trainingLoad)>0)
    .map(s=>({date:s.date,value:Number(s.trainingLoad),rpe:s.rpe,duration:s.totalSeconds,kind:s.kind})).sort((a,b)=>a.date.localeCompare(b.date));
}
