...
function handleAction(action,arg){
  const map={
    ...
    close:()=>closeModal(), dayPreview:()=>openSessionPreview(Number(arg)), history:()=>openSessionHistory(), resolveInjury:()=>resolveInjury(arg), supplementalPreview:()=>openSupplementalPreview(arg), supplementalStart:()=>requestSupplementalStart(arg), toggleSupplemental:()=>toggleSupplemental(arg)
  };
  map[action]?.();
}
...
function openSupplementalPreview(programId){
  ...
  if(p.type!=='supplemental')return;
  const def=p.buildSession();
  const blocked=activeBlockedTags(), affected=def.steps.filter(s=>restrictionMatches(s,blocked).restricted);
  const enabled=supplementalEnabled(p.id), due=isSupplementalDue(p), done=completedSupplementalToday(p.id);
  const unfinished=incompleteSupplementalSession(p.id), ready=latestReadiness(state,today()), next=nextSupplementalDate(p);
  const canStart=enabled&&due&&!done;

  showModal(`<div class="step-top"><div><div class="kicker">Optional program preview</div><h2>${esc(p.name)}</h2></div><button class="chip" data-action="close">Close</button></div>
    <p>${esc(def.goal)}</p>
    <div class="row wrap"><span class="chip">${esc(p.schedule?.label||'Recurring')}</span><span class="chip">~${secondsFmt(estimateSessionSeconds(def,state.settings.preCountdown))}</span><span class="chip">${def.steps.filter(s=>!s.isRest).length} work sets/steps</span>${ready?`<span class="chip ${ready.band==='green'?'good':ready.band==='yellow'?'warn':'bad'}">Readiness ${ready.score}</span>`:''}${affected.length?`<span class="chip warn">${affected.length} restricted</span>`:''}</div>
    ${p.progressionNote?`<div class="alert" style="margin-top:12px">${esc(p.progressionNote)}</div>`:''}
    ${affected.length?`<div class="alert warn" style="margin-top:12px">Active restrictions intersect this session. The runner will block those steps until you explicitly skip or override them.</div>`:''}
    ${!enabled?`<div class="alert" style="margin-top:12px">This optional program is installed but disabled. Enable it in Settings or the Program tab to schedule sessions.</div>`:
      done?`<div class="alert good" style="margin-top:12px">Today’s session is already complete. A second same-day session is intentionally not offered.</div>`:
      !due?`<div class="alert" style="margin-top:12px">Preview only today. ${esc(p.name)} is scheduled ${esc(p.schedule?.label||'on its configured days')}.${next?` Next: ${prettyDate(next)}.`:''}</div>`:''}
    <div class="section-title">Sequence</div><div class="list">${def.steps.map((s,i)=>{
      const r=restrictionMatches(s,blocked);return `<div class="list-item ${r.restricted?'restricted':''}"><div class="row between"><div><strong>${i+1}. ${esc(s.name)}</strong><div class="small">${esc(s.phase)} · ${esc(s.target)}</div></div>${s.isRest?'<span class="chip">REST</span>':r.restricted?'<span class="chip bad">RESTRICTED</span>':''}</div>${r.restricted?`<div class="tiny" style="margin-top:5px">Matched: ${r.matched.map(esc).join(', ')}</div>`:''}</div>`;
    }).join('')}</div>
    <div class="spacer12"></div>${canStart?`<button class="btn" id="supplementalPreviewStart">${unfinished?'Resume scheduled session':'Start scheduled session'}</button>`:''}`);
  if(canStart)$('#supplementalPreviewStart').addEventListener('click',()=>requestSupplementalStart(p.id));
}

function requestSupplementalStart(programId){
  const p=getProgram(programId);
  if(p.type!=='supplemental'||!supplementalEnabled(p.id)||!isSupplementalDue(p))return;
  if(completedSupplementalToday(p.id)){alert(`${p.name} is already complete for today.`);return;}
  const existing=incompleteSupplementalSession(p.id);
  if(existing){startRunner(existing);return;}
  const ready=latestReadiness(state,today());
  const proceed=()=>beginSupplementalSession(p);
  if(state.settings.readinessGate&&!ready)openReadiness(proceed);
  else if(ready?.band==='red'){
    if(confirm(`Today’s readiness entry recommends recovery (${ready.score}/100). Start ${p.name} anyway?`))proceed();
  }else proceed();
}

function beginSupplementalSession(p){
  if(completedSupplementalToday(p.id)||!isSupplementalDue(p)||!supplementalEnabled(p.id))return;
  for(const old of state.sessions.filter(s=>s.kind==='supplemental'&&s.programId===p.id&&s.date!==today()&&!s.completedAt&&!s.abandonedAt)){
    old.abandonedAt=nowIso();
    old.notes=[old.notes,'Automatically closed when a later scheduled session was started.'].filter(Boolean).join(' ');
  }
  const def=p.buildSession();
  const session={
    id:newId('session'),date:today(),kind:'supplemental',programId:p.id,programVersion:p.version,programDay:null,week:null,
    title:p.name,scheduledWeekday:dateWeekday(today()),startedAt:nowIso(),completedAt:null,abandonedAt:null,
    definition:def.steps,steps:[],segments:[],rpe:null,notes:'',activeSeconds:0,recoverySeconds:0,otherSeconds:0,totalSeconds:0,trainingLoad:null
  };
