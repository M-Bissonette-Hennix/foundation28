import { getProgram, listPrograms, listSupplementalPrograms, findExerciseById } from './program-registry.js';
...
import {
  RETURN_STAGES, returnStageInfo, returnAdjustmentForStep, adaptDefinition, recommendSessionScaling,
...
function supplementalConfig(programId){
  state.supplementalPrograms = state.supplementalPrograms || {};
  if(!state.supplementalPrograms[programId]){
    const p=getProgram(programId);
    state.supplementalPrograms[programId]={
      enabled:true,
      installedAt:nowIso(),
      scheduleWeekdays:[...(p.schedule?.weekdays||[])]
    };
  }
  return state.supplementalPrograms[programId];
}
function supplementalEnabled(programId){return supplementalConfig(programId).enabled!==false;}
function dateWeekday(dateString=today()){
  const [y,m,d]=String(dateString).split('-').map(Number);
  return new Date(y,m-1,d).getDay();
}
function isSupplementalDue(p,dateString=today()){
  const schedule=supplementalConfig(p.id).scheduleWeekdays?.length?supplementalConfig(p.id).scheduleWeekdays:(p.schedule?.weekdays||[]);
  return schedule.includes(dateWeekday(dateString));
}
function nextSupplementalDate(p,fromDate=today()){
  ...
  for(let offset=1;offset<=14;offset++){
    ...
    const s=isoDate(candidate);
    if(isSupplementalDue(p,s))return s;
  }
  return null;
}
function completedSupplementalToday(programId,dateString=today()){
  return state.sessions.find(s=>s.kind==='supplemental'&&s.programId===programId&&s.date===dateString&&s.completedAt)||null;
}
function incompleteSupplementalSession(programId,dateString=today()){
  return [...state.sessions].filter(s=>s.kind==='supplemental'&&s.programId===programId&&s.date===dateString&&!s.completedAt&&!s.abandonedAt)
    .sort((a,b)=>String(b.startedAt||'').localeCompare(String(a.startedAt||'')))[0]||null;
}
function sessionDisplayName(session){
  if(!session)return 'Session';
  if(session.kind==='recovery')return 'Recovery session';
  if(session.kind==='supplemental')return getProgram(session.programId)?.name||session.title||'Supplemental session';
  return `Program Day ${session.programDay}`;
}
function allExerciseOptions(){
  const seen=new Map();
  for(const p of [program,...listSupplementalPrograms()]){
    for(const e of Object.values(p.exercises||{})){
      if(!String(e.id).startsWith('recovery-'))seen.set(e.id,e);
    }
  }
  return [...seen.values()].sort((a,b)=>a.name.localeCompare(b.name));
}
function completedProgramToday(dateString=today()){
  return [...state.sessions].filter(s=>s.kind==='program'&&s.date===dateString&&s.completedAt).sort((a,b)=>String(b.completedAt).localeCompare(String(a.completedAt)))[0]||null;
}
function completedWalkToday(dateString=today()){
  return [...state.walks].filter(w=>w.date===dateString&&w.completedAt).sort((a,b)=>String(b.completedAt).localeCompare(String(a.completedAt)))[0]||null;
}
function todayCoordinationItems(){
  const items=[];
  const primaryDone=completedProgramToday();
  if(primaryDone?.definition?.length)items.push({id:program.id,name:program.name,kind:'program',status:'completed',definition:{steps:primaryDone.definition}});
  else if(state.programState.status==='active')items.push({id:program.id,name:program.name,kind:'program',status:'planned',definition:program.buildSession(currentDay())});
  for(const p of listSupplementalPrograms().filter(x=>supplementalEnabled(x.id)&&isSupplementalDue(x))){
    const done=completedSupplementalToday(p.id);
    items.push({id:p.id,name:p.name,kind:'supplemental',status:done?'completed':'planned',definition:done?.definition?.length?{steps:done.definition}:p.buildSession()});
  }
  return items;
}
function currentCollision(){
  if(!state.settings.collisionManagement)return {severity:'none',score:0,overlaps:[],topMuscles:[],programCount:0};
  return collisionAnalysis(todayCoordinationItems());
}
function recommendationFor(definition,kind){
  if(!state.settings.adaptiveScaling)return {factor:1,restMultiplier:1,reasons:[],recommendRecovery:false,readiness:latestReadiness(state,today()),load:loadWindowAnalytics(state)};
  return recommendSessionScaling(state,definition,{kind,collision:currentCollision(),date:today()});
}
function materializeDefinition(definition,kind,mode='recommended'){
  const rec=recommendationFor(definition,kind);
  if(mode==='full')return adaptDefinition(definition,state,{factor:1,restMultiplier:1,reasons:['User bypassed readiness/collision reduction; injury-return stages remain active']});
  return adaptDefinition(definition,state,{factor:rec.factor,restMultiplier:rec.restMultiplier,reasons:rec.reasons});
}
function fmtChange(v){return v===null||!Number.isFinite(Number(v))?'No prior window':`${v>0?'+':''}${Math.round(v)}% vs prior`;}
...
function renderCommandCenter(){
  ...
  const pendingSeconds=pending.reduce((a,x)=>a+estimateSessionSeconds(x.definition,state.settings.preCountdown),0)+(walkDone?0:Number(state.settings.walkGoalMinutes||0)*60);
  ...
  return `<div class="card ${ready?.band==='red'?'bad':collisionClass(collision.severity)}">
    <div class="row between"><div><div class="kicker">Daily command center</div><h2>${prettyDate(today())}</h2></div><span class="chip ${ready?.band==='green'?'good':ready?.band==='yellow'?'warn':ready?.band==='red'?'bad':''}">${ready?`Readiness ${ready.score}`:'Readiness —'}</span></div>
    <div class="grid2">
      <div class="stat"><strong>${secondsFmt(pendingSeconds)}</strong><span>Estimated remaining</span></div>
      <div class="stat"><strong>${load.current7}</strong><span>7-day load · ${esc(fmtChange(load.change7))}</span></div>
...
function renderSupplementalTodayCards(){
  return listSupplementalPrograms().filter(p=>supplementalEnabled(p.id)).map(p=>{
    const due=isSupplementalDue(p), done=completedSupplementalToday(p.id), unfinished=incompleteSupplementalSession(p.id);
    const next=nextSupplementalDate(p);
    const def=p.buildSession();
    const affected=def.steps.filter(s=>restrictionMatches(s,activeBlockedTags()).restricted).length;
    return `<div class="card ${done?'good':due?'':'subtle'}">
      <div class="row between"><div><div class="kicker">Optional add-on · ${esc(p.schedule?.label||'Recurring')}</div><h2>${esc(p.name)}</h2></div><span class="chip ${done?'good':due?'warn':''}">${done?'DONE TODAY':due?'SCHEDULED TODAY':'OFF DAY'}</span></div>
      <p>${esc(p.description)}</p>
      <div class="row wrap"><span class="chip">~${secondsFmt(estimateSessionSeconds(def,state.settings.preCountdown))}</span><span class="chip">${def.steps.filter(s=>!s.isRest).length} work sets/steps</span>${affected?`<span class="chip warn">${affected} restricted</span>`:''}</div>
      <div class="spacer12"></div>
      ${done?`<div class="alert good">Today’s ${esc(p.name)} session is complete. The normal UI will not schedule a second one today.</div>`:
        due?`<button class="btn ${unfinished?'good':''}" data-action="${unfinished?'supplementalStart':'supplementalPreview'}" data-arg="${esc(p.id)}">${unfinished?'Resume arm session':'Preview arm session'}</button>`:
        `<div class="alert">Not scheduled today. Next session: ${next?prettyDate(next):esc(p.schedule?.label||'scheduled day')}.</div><div class="spacer8"></div><button class="btn secondary" data-action="supplementalPreview" data-arg="${esc(p.id)}">Preview routine</button>`}
    </div>`;
  }).join('');
}

function renderSupplementalProgramCards(){
  return listSupplementalPrograms().map(p=>{
    const enabled=supplementalEnabled(p.id), due=isSupplementalDue(p), next=nextSupplementalDate(p), def=p.buildSession();
    const completed=state.sessions.filter(s=>s.kind==='supplemental'&&s.programId===p.id&&s.completedAt).length;
    return `<div class="card">
      <div class="row between"><div><div class="kicker">Optional recurring program</div><h2>${esc(p.name)}</h2></div><span class="chip ${enabled?'good':''}">${enabled?'ENABLED':'DISABLED'}</span></div>
      <p>${esc(p.description)}</p>
      <div class="row wrap"><span class="chip">${esc(p.schedule?.label||'Recurring')}</span><span class="chip">${completed} completed</span><span class="chip">~${secondsFmt(estimateSessionSeconds(def,state.settings.preCountdown))}</span></div>
      ${p.progressionNote?`<div class="alert" style="margin-top:12px">${esc(p.progressionNote)}</div>`:''}
      <div class="spacer12"></div><button class="btn secondary" data-action="supplementalPreview" data-arg="${esc(p.id)}">${due&&enabled?'Preview today’s session':'Inspect routine'}</button>
      <div class="spacer8"></div><button class="btn ghost" data-action="toggleSupplemental" data-arg="${esc(p.id)}">${enabled?'Disable add-on':'Enable add-on'}</button>
      ${enabled&&!due&&next?`<div class="small" style="margin-top:8px">Next scheduled session: ${prettyDate(next)}</div>`:''}
    </div>`;
  }).join('');
}

function renderInstalledProgramsSettings(){
  return listPrograms().map(p=>{
    const supplemental=p.type==='supplemental';
    const enabled=supplemental?supplementalEnabled(p.id):true;
    return `<div class="list-item"><div class="row between"><div><strong>${esc(p.name)}</strong><div class="small">${esc(p.description)}</div>${p.schedule?.label?`<div class="tiny" style="margin-top:4px">${esc(p.schedule.label)}</div>`:''}</div><span class="chip ${enabled?'good':''}">v${esc(p.version)}</span></div>
      ${supplemental?`<div class="spacer8"></div><button class="btn ghost" data-action="toggleSupplemental" data-arg="${esc(p.id)}">${enabled?'Disable optional program':'Enable optional program'}</button>`:`<div class="tiny" style="margin-top:7px">Primary program · remains installed and active independently.</div>`}
    </div>`;
  }).join('');
}

function toggleSupplemental(programId){
  const p=getProgram(programId);
  if(p.type!=='supplemental')return;
  const cfg=supplementalConfig(programId);
...
function renderToday(){
  ...
  const primaryDoneToday=completedProgramToday();
  ...
  return `
    ...
    ${lastSession?`<div class="card subtle"><div class="kicker">Most recent completed session</div><div class="row between"><div><h3>${esc(sessionDisplayName(lastSession))}</h3><div class="small">${prettyDate(lastSession.date)} · ${secondsFmt(lastSession.totalSeconds||0)}${lastSession.rpe?` · RPE ${lastSession.rpe}`:''}</div></div>${lastSession.trainingLoad?`<span class="chip">Load ${lastSession.trainingLoad}</span>`:''}</div></div>`:''}
  `;
}
...
function renderProgram(){
  ...
  return `
    ...
    <div class="section-title">Optional installed programs</div>
    ${renderSupplementalProgramCards()}
  `;
}
...
function injuryCard(i){
  const info=returnStageInfo(i.returnStage);
  return `<div class="list-item restricted"><div class="row between"><div><strong>${esc(i.side||'')} ${esc(i.area)}</strong><div class="small">Severity ${i.severity}/10 · since ${prettyDate(i.date)}</div></div>${returnStageChip(i)}</div><div class="small" style="margin-top:6px">Return stage ${Number(i.returnStage)||0}: ${esc(info.label)} · ${esc(info.description)}</div><div class="small" style="margin-top:5px">Affected categories: ${(i.blockedTags||[]).map(esc).join(', ')||'No tags selected'}</div>${i.notes?`<div class="small" style="margin-top:5px">${esc(i.notes)}</div>`:''}<div class="spacer8"></div><div class="grid2">${Number(i.returnStage)>0?`<button class="btn ghost" data-action="injuryRegress" data-arg="${esc(i.id)}">Previous stage</button>`:'<button class="btn ghost" disabled>Previous stage</button>'}${Number(i.returnStage)<4?`<button class="btn secondary" data-action="injuryAdvance" data-arg="${esc(i.id)}">Advance stage</button>`:`<button class="btn secondary" data-action="resolveInjury" data-arg="${esc(i.id)}">Resolve</button>`}</div>${Number(i.returnStage)<4?`<div class="spacer8"></div><button class="btn ghost" data-action="resolveInjury" data-arg="${esc(i.id)}">Resolve now</button>`:''}</div>`;
}
function renderMetrics(){
  const weights=weightSeries(state.weights);
  const loads=sessionLoadSeries(state);
  const load=loadWindowAnalytics(state);
  const volume7=muscleVolumeWindow(state,7), volume28=muscleVolumeWindow(state,28);
  const completed=state.sessions.filter(s=>s.completedAt);
  const programSessions=completed.filter(s=>s.kind==='program');
  const supplementalSessions=completed.filter(s=>s.kind==='supplemental');
  ...
  const exerciseOptions=allExerciseOptions();
  ...
  return `
    ...
    <div class="card"><div class="kicker">7 / 28-day workload</div><h2>Training-load windows</h2><p class="small">Completed-session load = duration in minutes × session RPE. Windows are descriptive workload totals, not injury-risk thresholds.</p>
      ...
      <div class="spacer12"></div>${barChart(load.daily,{unit:''})}
    </div>

    <div class="card"><div class="kicker">Muscle-group volume</div><h2>Set-equivalent accounting</h2><p class="small">A heuristic accounting layer based on completed work steps and each exercise’s primary/secondary muscle contribution. Fractions represent secondary involvement; they are not literal anatomical set counts.</p>
      ${muscleRows.length?muscleRows.map(m=>`<div class="bar-row"><span class="bar-label">${esc(m)}</span><div class="bar"><i style="width:${Math.round(((volume7[m]||0)/maxMuscle)*100)}%"></i></div><span class="bar-value">${(volume7[m]||0).toFixed(1)} / ${(volume28[m]||0).toFixed(1)}</span></div>`).join(''):'<div class="alert">No completed work-step volume yet.</div>'}
      <div class="small" style="margin-top:9px">Values shown as 7-day / 28-day set-equivalents.</div>
    </div>
    ...
    <div class="card"><div class="kicker">Training load</div><h2>Individual sessions</h2>${barChart(loads.slice(-20),{unit:''})}${loads.length?`<div class="small">Most recent load: ${loads[loads.length-1].value} AU · ${prettyDate(loads[loads.length-1].date)}</div>`:''}</div>
...
function renderSettings(){
  return `
    ...
    <div class="card"><div class="kicker">Runner</div>
      ...
      <label class="toggle"><span>Adaptive session recommendations</span><input type="checkbox" data-setting="adaptiveScaling" ${state.settings.adaptiveScaling?'checked':''}></label>
      <label class="toggle"><span>Multi-program collision management</span><input type="checkbox" data-setting="collisionManagement" ${state.settings.collisionManagement?'checked':''}></label>
...
const RESTRICTIONS=[
  ...
  ['lower-body','Lower-body work'],['balance','Balance-demanding work'],['wrist-load','Wrist loading'],['shoulder-load','Shoulder loading'],['elbow-load','Elbow loading'],
  ['upper-push','Upper-body pushing'],['upper-pull','Upper-body pulling'],['forearm-load','Forearm loading'],
...
const PRESETS={
  ...
  'Shoulder':['shoulder-load','upper-push','upper-pull','hanging'], 'Elbow':['elbow-load','upper-push','upper-pull','hanging'], 'Wrist / hand':['wrist-load','forearm-load','upper-push','upper-pull','grip','hanging']
};
...
function openInjuryManager(){
  ...
  showModal(`<div class="step-top"><div><div class="kicker">Movement restriction system</div><h2>Injuries / limitations</h2></div><button class="chip" data-action="close">Close</button></div>
    <div class="alert">These records are user-defined training restrictions. Return stages are manually advanced: Blocked → Regression → 50% → 75% → Full/monitoring. The app never advances a stage automatically and does not identify a diagnosis or determine whether exercise is medically safe.</div>
...
function openAddInjury(){
  ...
  $('#saveInjury').addEventListener('click',()=>{
    const record={id:newId('injury'),date:today(),createdAt:nowIso(),area:$('#injuryArea').value,side:$('#injurySide').value,severity:Number($('#injurySeverity').value),blockedTags:$$('#restrictionChecks input:checked').map(x=>x.value),notes:$('#injuryNotes').value.trim(),status:'active',returnStage:0,returnHistory:[{stage:0,at:nowIso(),date:today(),reason:'Created'}],resolvedAt:null,resolvedDate:null};
    state.injuries.push(record);persist();openInjuryManager();render();
  });
}

function changeInjuryStage(id,delta){
  const i=state.injuries.find(x=>x.id===id);if(!i||i.status!=='active')return;
  const current=Math.min(4,Math.max(0,Number(i.returnStage)||0)), next=Math.min(4,Math.max(0,current+Number(delta||0)));
  if(next===current)return;
  const from=returnStageInfo(current),to=returnStageInfo(next);
  if(!confirm(`Move ${i.side?i.side+' ':''}${i.area} from ${from.label} to ${to.label}? This changes how matching exercises are presented/scaled; it is not a medical clearance.`))return;
  i.returnStage=next;i.returnHistory=[...(i.returnHistory||[]),{stage:next,from:current,at:nowIso(),date:today(),reason:'Manual stage change'}];persist();openInjuryManager();render();
}

function resolveInjury(id){
  const i=state.injuries.find(x=>x.id===id);if(!i)return;
  if(confirm(`Mark ${i.side?i.side+' ':''}${i.area} as resolved?`)){i.status='resolved';i.resolvedAt=nowIso();i.resolvedDate=today();i.returnHistory=[...(i.returnHistory||[]),{stage:'resolved',from:Number(i.returnStage)||0,at:nowIso(),date:today(),reason:'Resolved'}];persist();openInjuryManager();render();}
}
...
function openSessionPreview(day=currentDay()){
  const raw=program.buildSession(day), blocked=activeBlockedTags(), estimated=estimateSessionSeconds(raw,state.settings.preCountdown);
  const affected=raw.steps.filter(step=>restrictionMatches(step,blocked).restricted), ready=latestReadiness(state,today()), collision=currentCollision(), rec=recommendationFor(raw,'program');
  const doneToday=completedProgramToday(), canStart=day===currentDay()&&state.programState.status==='active'&&!doneToday;
...
function requestProgramStart(mode='recommended'){
  if(state.programState.status!=='active'||completedProgramToday())return;
  const existing=incompleteSession('program',currentDay());
  if(existing){startRunner(existing);return;}
  const proceed=()=>beginProgramSession(mode);
  if(state.settings.readinessGate&&!latestReadiness(state,today()))openReadiness(proceed); else proceed();
}
...
function openSupplementalPreview(programId){
  const p=getProgram(programId);if(p.type!=='supplemental')return;
  const raw=p.buildSession(),blocked=activeBlockedTags(),affected=raw.steps.filter(step=>restrictionMatches(step,blocked).restricted);
  const enabled=supplementalEnabled(p.id),due=isSupplementalDue(p),done=completedSupplementalToday(p.id),unfinished=incompleteSupplementalSession(p.id),ready=latestReadiness(state,today()),next=nextSupplementalDate(p),collision=currentCollision(),rec=recommendationFor(raw,'supplemental'),adapted=materializeDefinition(raw,'supplemental','recommended');
  const canStart=enabled&&due&&!done;
  showModal(`<div class="step-top"><div><div class="kicker">Optional program preview</div><h2>${esc(p.name)}</h2></div><button class="chip" data-action="close">Close</button></div><p>${esc(raw.goal)}</p>
  ...
  $('#suppRecovery')?.addEventListener('click',openRecoveryPreview);
  $('#suppRecommended')?.addEventListener('click',()=>requestSupplementalStart(p.id,'recommended'));
  $('#suppFull')?.addEventListener('click',()=>requestSupplementalStart(p.id,'full'));
}

function requestSupplementalStart(programId,mode='recommended'){
  const p=getProgram(programId);if(p.type!=='supplemental'||!supplementalEnabled(p.id)||!isSupplementalDue(p)||completedSupplementalToday(p.id))return;
  const existing=incompleteSupplementalSession(p.id);if(existing){startRunner(existing);return;}
  const proceed=()=>beginSupplementalSession(p,mode);
  if(state.settings.readinessGate&&!latestReadiness(state,today()))openReadiness(proceed);else proceed();
}

function beginSupplementalSession(p,mode='recommended'){
  if(completedSupplementalToday(p.id)||!isSupplementalDue(p)||!supplementalEnabled(p.id))return;
  for(const old of state.sessions.filter(s=>s.kind==='supplemental'&&s.programId===p.id&&s.date!==today()&&!s.completedAt&&!s.abandonedAt)){old.abandonedAt=nowIso();old.notes=[old.notes,'Automatically closed when a later scheduled session was started.'].filter(Boolean).join(' ');}
  const raw=p.buildSession(),def=materializeDefinition(raw,'supplemental',mode),rec=recommendationFor(raw,'supplemental');
  if(mode==='recommended'&&rec.recommendRecovery&&!confirm('Today’s readiness recommends recovery. Continue with the conservative scaled add-on anyway?')){openRecoveryPreview();return;}
  const session={id:newId('session'),date:today(),kind:'supplemental',programId:p.id,programVersion:p.version,programDay:null,week:null,title:p.name,scheduledWeekday:dateWeekday(today()),startedAt:nowIso(),completedAt:null,abandonedAt:null,sourceDefinition:raw.steps,definition:def.steps,adaptivePlan:{mode,...def.adaptivePlan},steps:[],segments:[],rpe:null,notes:'',activeSeconds:0,recoverySeconds:0,otherSeconds:0,totalSeconds:0,trainingLoad:null};
...
function renderRunnerStep(){
  ...
  const runnerLabel=session.kind==='recovery'?'RECOVERY':session.kind==='supplemental'?getProgram(session.programId).name.toUpperCase():`DAY ${session.programDay}`;
  const base=`<div class="step-top"><div class="step-count">${esc(runnerLabel)} · STEP ${runner.index+1}/${steps.length} · ${esc(step.phase)}</div><button class="chip" id="exitRunner">Exit</button></div>
    ...
    ${match.restricted&&!overridden?`<div class="alert bad" style="margin-top:12px"><strong>ACTIVE RESTRICTION</strong><br>This movement matches: ${match.matched.map(esc).join(', ')}.</div>`:''}
    ${step.forceRegression?`<div class="alert warn" style="margin-top:12px"><strong>RETURN STAGE 1:</strong> Use the listed regression for this movement. Do not treat the app’s progression stage as medical clearance.</div>`:''}
    ${Number(step.adaptiveFactor)>0&&Number(step.adaptiveFactor)<1?`<div class="small" style="margin-top:8px">Adaptive dose: ${Math.round(Number(step.adaptiveFactor)*100)}% of the stored prescription.</div>`:''}<hr>`;
...
function renderTimedStep(base,session,step){
  const ranged=Number(step.minSeconds)>0&&Number(step.minSeconds)<Number(step.seconds);
  showModal(base+timerMarkup(step.seconds)+`${ranged?`<div class="alert" style="margin-bottom:12px">Target range: complete any time from ${Number(step.minSeconds)} to ${Number(step.seconds)} seconds.</div>`:''}<button class="btn" id="timerMain">Start</button>${ranged?`<div class="spacer8"></div><button class="btn secondary" id="completeTimedRange" disabled>Minimum not reached</button>`:''}<div class="spacer8"></div><button class="btn ghost" id="skipStep">Skip / stop</button><div class="spacer8"></div><button class="btn ghost" id="runnerPain">Log pain / issue</button>`,{lock:true});
  $('#timerMain').addEventListener('click',()=>startCountdownAndTimer(step,{
    onTick:elapsed=>{
      if(ranged&&elapsed>=Number(step.minSeconds)){
        const b=$('#completeTimedRange');
        if(b){b.disabled=false;b.textContent='Complete hold now';}
      }
    },
    onComplete:()=>completeRunnerStep(step,{actualSeconds:step.seconds}),
    isRest:Boolean(step.isRest)
  }));
  $('#completeTimedRange')?.addEventListener('click',()=>{
    const elapsed=Math.round(timerController?.getElapsed?.()||0);
    if(elapsed<Number(step.minSeconds))return;
    stopTimerController();
    completeRunnerStep(step,{actualSeconds:Math.min(Number(step.seconds),Math.max(Number(step.minSeconds),elapsed))});
  });
  $('#skipStep').addEventListener('click',()=>skipRunnerStep(step,'user'));
}
...
function finishSession(session){
  ...
  const finishTitle=session.kind==='program'?`Day ${session.programDay} finished`:session.kind==='supplemental'?`${getProgram(session.programId).name} finished`:'Recovery session finished';
  showModal(`<div class="kicker">Session complete</div><h2>${esc(finishTitle)}</h2>
  ...
  $('#saveSessionFinish').addEventListener('click',()=>{
    ...
    if(session.kind==='program')advanceProgram(state,session.date,program.durationDays);
...
function openExerciseHistory(exerciseId){
  const exercise=findExerciseById(exerciseId);if(!exercise)return;
...
function openSessionHistory(){
  ...
  showModal(`<div class="step-top"><div><div class="kicker">Training history</div><h2>Completed sessions</h2></div><button class="chip" data-action="close">Close</button></div>
    ${rows.length?`<div class="list">${rows.map(s=>`<div class="list-item"><div class="row between"><div><strong>${esc(sessionDisplayName(s))}</strong><div class="small">${prettyDate(s.date)} · ${secondsFmt(s.totalSeconds||0)}${s.rpe?` · RPE ${s.rpe}`:''}</div></div>${s.trainingLoad?`<span class="chip">Load ${s.trainingLoad}</span>`:''}</div><div class="small" style="margin-top:6px">Active ${secondsFmt(s.activeSeconds||0)} · Recovery ${secondsFmt(s.recoverySeconds||0)} · Other ${secondsFmt(s.otherSeconds||0)}${s.adaptivePlan?.factor&&s.adaptivePlan.factor<1?` · Adaptive ${Math.round(s.adaptivePlan.factor*100)}%`:''}</div>${s.notes?`<div class="small" style="margin-top:5px">${esc(s.notes)}</div>`:''}</div>`).join('')}</div>`:'<p class="small">No completed sessions yet.</p>'}`);
}
