...
function normalize(state) {
  ...
  for (const [programId, defaults] of Object.entries(base.supplementalPrograms)) {
    ...
    s.supplementalPrograms[programId] = {
      ...defaults,
      ...prior,
      enabled: prior.enabled !== false,
      scheduleWeekdays: Array.isArray(prior.scheduleWeekdays) && prior.scheduleWeekdays.length
        ? prior.scheduleWeekdays.map(Number).filter(n => Number.isInteger(n) && n >= 0 && n <= 6)
  ...
  s.injuries = s.injuries.map(i => ({
    ...i,
    returnStage: Math.min(4,Math.max(0,Number.isFinite(Number(i.returnStage))?Number(i.returnStage):0)),
    returnHistory: Array.isArray(i.returnHistory)?i.returnHistory:[]
  }));
